...
namespace NiceHashMiner
{
    ...
    public class NiceHashData
    {
        ...
        public NiceHashData() {
            ...
            foreach (AlgorithmType algo in Enum.GetValues(typeof(AlgorithmType))) {
                if (algo >= 0) {
                    ...
                    _recentPaying[algo] = new List<double> { 0 };
                }
            }
            _currentSma = sma;
        }
        ...
        public Dictionary<AlgorithmType, NiceHashSma> NormalizedSMA() {
            foreach (AlgorithmType algo in _recentPaying.Keys) {
        ...
        private double currentPayingForAlgo(AlgorithmType algo) {
            if (_recentPaying.ContainsKey(algo))
                return _recentPaying[algo].LastOrDefault();
            return 0;
        }
    }
}
