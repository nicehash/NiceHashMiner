    ...
    public class ComputeDeviceManager
    {
        public static class Query
        {
            ...
            public static void QueryDevices(IMessageNotifier messageNotifier)
            {
                ...
                {
                    ShowMessageAndStep(Translations.Tr("Querying CUDA devices"));
                ...
                if (ConfigManager.GeneralConfig.DeviceDetection.DisableDetectionAMD)
                {
                    ...
                    ShowMessageAndStep(Translations.Tr("Skip check for AMD OpenCL GPUs"));
                }
                else
                {
                    // #3 OpenCL
                    ShowMessageAndStep(Translations.Tr("Querying OpenCL devices"));
                    OpenCL.QueryOpenCLDevices();
                    // #4 AMD query AMD from OpenCL devices, get serial and add devices
                    ShowMessageAndStep(Translations.Tr("Checking AMD OpenCL GPUs"));
                    ...
                    !isNvidiaErrorShown && _currentNvidiaSmiDriver.LeftPart > -1)
                {
                    ...
                    var nvdriverString = _currentNvidiaSmiDriver.LeftPart > -1
                        ? string.Format(
                            Translations.Tr(" (current {0})"),
                            _currentNvidiaSmiDriver)
                        : "";
                    MessageBox.Show(string.Format(
                            Translations.Tr("We have detected that your Nvidia Driver is older than {0}{1}. We recommend you to update to {2} or newer."),
                ...
                {
                    var result = MessageBox.Show(Translations.Tr("No supported devices are found. Select the OK button for help or cancel to continue."),
                        Translations.Tr("No Supported Devices"),
                ...
                if (ConfigManager.GeneralConfig.ShowDriverVersionWarning && totalSysRam < totalGpuRam)
                {
                    ...
                    MessageBox.Show(Translations.Tr("NiceHash Miner Legacy recommends increasing virtual memory size so that all algorithms would work fine."),
            ...
            private static class WindowsDisplayAdapters
            {
                    ...
                    bool warningsEnabled)
                {
                    ...
                    if (warningsEnabled)
                    {
                        ...
                        {
                            var msg = Translations.Tr("We have detected a Video Controller that is not working properly. NiceHash Miner Legacy will not be able to use this Video Controller for mining. We advise you to restart your computer, or reinstall your Video Controller drivers.");
                            foreach (var vc in avaliableVideoControllers)
                            {
                                if (!vc.Status.ToLower().Equals("ok"))
                                {
                                    msg += Environment.NewLine
                                           + string.Format(
                                               Translations.Tr("Name: {0}, Status {1}, PNPDeviceID {2}"),
                                               vc.Name, vc.Status, vc.PnpDeviceID);
                                }
                            }
                            MessageBox.Show(msg,
                                Translations.Tr("Warning! Video Controller not operating correctly"),
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    }
                }
            ...
            private static class Cpu
            {
                public static void QueryCpus()
                {
                    ...
                    if (!Helpers.Is64BitOperatingSystem)
                    {
                        ...
                        {
                            MessageBox.Show(Translations.Tr("NiceHash Miner Legacy works only on 64-bit version of OS for CPU mining. CPU mining will be disabled."),
                                Translations.Tr("Warning!"),
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    ...
                    if (threadsPerCpu * Available.CpusCount > 64)
                    {
                        if (ConfigManager.GeneralConfig.ShowDriverVersionWarning)
                        {
                            MessageBox.Show(Translations.Tr("NiceHash Miner Legacy does not support more than 64 virtual cores. CPU mining will be disabled."),
                                Translations.Tr("Warning!"),
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
